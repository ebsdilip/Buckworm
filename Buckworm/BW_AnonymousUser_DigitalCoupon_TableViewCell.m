//
//  BW_AnonymousUser_DigitalCoupon_TableViewCell.m
//  buckworm
//
...
//        imgViewStackedOffer.image = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"stack1" ofType:@"png"]];
        ...
        btnViewAllOffers = [[UIButton alloc] initWithFrame:CGRectMake(182, 2, 125, 125)];
        [btnViewAllOffers addTarget:self action:@selector(showAllOffers) forControlEvents:UIControlEventTouchUpInside];
        btnViewAllOffers.backgroundColor = [UIColor clearColor];
        btnViewAllOffers.titleLabel.font = [UIFont systemFontOfSize:10.0];
        btnViewAllOffers.titleLabel.adjustsFontSizeToFitWidth = YES;
        [btnViewAllOffers setImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"multipleoffer" ofType:@"png"]] forState:UIControlStateNormal];
    }
    return self;
}
...
- (void)showAllOffers
{
    ...
    if(viewAllOffer==nil)
        viewAllOffer = [[BWShowAllOfferView alloc] initWithFrame:CGRectMake(0, 600, 320, 568)];
    else
        [viewAllOffer removeFromSuperview];
    viewAllOffer.arrOffers = arrOffers;
    NSDictionary *dictOffer = [arrOffers lastObject];
    NSDictionary *dictMerchant = [dictOffer objectForKey:@"merchant"];

    viewAllOffer.lblHeaderTitle.text = [[dictMerchant objectForKey:@"name"]?[[dictMerchant objectForKey:@"name"] uppercaseString]:@"" stringByReplacingOccurrencesOfString:@"&amp;" withString:@"&"];
    viewAllOffer.strImagePath = strImagePath;
    viewAllOffer.backgroundColor = colorTableBG;
    [appDelegate.window addSubview:viewAllOffer];
    
    [UIView beginAnimations:nil context:NULL];
    [UIView setAnimationDuration:0.5];
    [UIView setAnimationDelegate:nil];
    viewAllOffer.frame = CGRectMake(0, 0, 320, 568);
...
//        appDelegate.currentLocationPoint.coordinate.longitude
    ...
    if([arrOffers count]>1)
        [self addSubview:btnViewAllOffers];
    else
        [btnViewAllOffers removeFromSuperview];
...
- (void)showOfferImage:(NSDictionary *)dictOffer
{
    if(isSingle==YES)
    {
        if([arrOffers count]>1)
        {
            ...
            imgViewStackedOffer.backgroundColor = [UIColor clearColor];
            imgViewStackedOffer.image = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"stack%i", [arrOffers count]<=6?[arrOffers count]:6] ofType:@"png"]];

            lblMerchant.frame = CGRectMake(20, diff-5, 200, 40);
            imgViewOfferType.frame = CGRectMake(230, 3+diff, 20, 20);
            lblOfferTypeTitle.frame = CGRectMake(255, diff, 40, 30);
            
            imgViewOffer.frame = CGRectMake(14, 40+diff, 298-diff, 120-diff);
            imgViewOfferShadow.frame = CGRectMake(14, diff, 298-diff, 160-diff);
            btnViewAllOffers.frame = CGRectMake(182, 2, 125, 125);

            if([arrOffers count]>=3)
            {
                imgViewOfferType.frame = CGRectMake(220, 3+diff, 20, 20);
                lblOfferTypeTitle.frame = CGRectMake(245, diff, 40, 30);
                lblBoughtTitle.frame = CGRectMake(195, 160, 70, 40);
                lblBoughtValue.frame = CGRectMake(270, 160, 30, 40);
            }
        }
    }
